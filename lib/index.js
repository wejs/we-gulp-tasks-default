...
module.exports = function initGulpTasks(Wee, gulp, projectFolder, done) {
  if (!Wee.bootstrap) {
    we = new Wee();
  } else {
    we = Wee;
  ...
  var buildFolder = path.join(projectFolder, 'files/public/build');
  ...
  gulp.task('footerProdJS',['loadWejs'], function() {
    return gulp.src(we.view.assets.getFileList('js', 'footer'))
      .pipe(gp_sourcemaps.init())
  ...
  gulp.task('cacheAllTemplates',['loadWejs'], function (cb) {
    we.view.cacheAllTemplates(we, cb);
  });

  gulp.task('apidoc',function (done) {
    apidoc.createDoc({
      src: ['node_modules/we-plugin-event/server'],
      dest: we.projectPath+'/documentation/',
      debug: false,
      includeFilters: [ '.*\\.js$' ]
    });
